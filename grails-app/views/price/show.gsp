	...
	<body>
		...
		<div id="show-price" class="content scaffold-show" role="main">
			...
			<g:if test="${flash.error}">
    			<div class="error" role="alert" style="display:block; color:red;margin-left: 30%;margin-bottom: 0%;">${flash.error}</div>    			
				</g:if>
			<ol class="property-list price">
				...
				<g:if test="${priceInstance?.product?.productName}">
				<li class="fieldcontain">
					<span id="product-label" class="property-label"><g:message code="price.product.label" default="Product" /></span>
					
						<span class="property-value" aria-labelledby="product-label"><g:link controller="product" action="show" id="${priceInstance?.product?.barcode}">${priceInstance?.product?.productName?.encodeAsHTML()}</g:link></span>
